...
export const ChatInterface: React.FC<Props> = ({ profile, onStateChange, isFullscreen, toggleFullscreen }) => {
  ...
  const [attachments, setAttachments] = useState<Attachment[]>(() => {
    try {
      const saved = sessionStorage.getItem('waifu_attachments_temp');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed)) return parsed;
      }
    } catch (e) {
      console.error("Failed to load temp attachments", e);
    }
    return [];
  });
  ...
  const handleResetLoadingState = () => {
      ...
      setShowRecoveryBtn(false);
      if (onStateChange) onStateChange('IDLE');
      console.log(">> MANUAL RECOVERY: UPLINK RESET & REQUEST ABORTED.");
  };
  ...
  // Periodic Auto-Save Heartbeat (Every 30s)
  useEffect(() => {
    const interval = setInterval(() => {
      console.log(">> HEARTBEAT: RUNNING AUTO-SAVE SYNC...");
      saveHistoryToStorage(messages);
      
      // Also sync chronicle just in case
      try {
        localStorage.setItem('waifu_chronicle', JSON.stringify(chronicle));
      } catch(e) {}
    }, 30000);
    return () => clearInterval(interval);
  }, [messages, chronicle, saveHistoryToStorage]);
  ...
  useEffect(() => {
    try {
        sessionStorage.setItem('waifu_attachments_temp', JSON.stringify(attachments));
    } catch (e) {
        console.warn(">> ATTACHMENT TEMP SAVE FAILED (QUOTA).");
    }
  }, [attachments]);
  ...
  const handleSendMessage = async (text: string) => {
      ...
      try {
          ...
          const responseText = await chatWithWaifu(profile, messages.concat(userMsg), text, attachments, chronicle, gameContext || '', abortControllerRef.current.signal);
          ...
          let errMsg = `>> TRANSMISSION ERROR: ${err.message || "UPLINK FAILURE"}`;
  ...
  const handleSummonResponse = async () => {
    ...
    try {
        ...
        const responseText = await chatWithWaifu(profile, messages, "[SUMMON_RESPONSE_TRIGGER]", [], chronicle, gameContext || '', abortControllerRef.current.signal);
        const aiMsg: Message = { id: (Date.now() + 1).toString(), role: 'waifu', content: responseText, timestamp: Date.now(), channel: 'RADIO' };
        setMessages(prev => [...prev, aiMsg]);
        if (onStateChange) onStateChange('SPEAKING');
        if (autoTTS) {
        ...
        setMessages(prev => [...prev, { id: Date.now().toString(), role: 'waifu', content: `>> UPLINK ERROR: ${err.message || "FAILED TO SUMMON RESPONSE."}`, timestamp: Date.now(), channel: 'RADIO' }]);
    } finally {
        setIsLoading(false);
        setTimeout(() => { if (!isSpeaking && onStateChange) onStateChange('IDLE'); }, 2000);
    }
    ...
    <div className={`flex flex-col h-full text-blue-200 font-mono relative transition-colors duration-300 ${getBgClass()}`}>
        <div className="flex border-b border-blue-900/30 shrink-0 relative bg-black/40">
            ...
            <div className="absolute right-14 top-1/2 -translate-y-1/2 flex items-center gap-2 pointer-events-none">
                <div className={`text-[8px] font-bold tracking-tighter ${saveStatus === 'SYNCED' ? 'text-green-500' : saveStatus === 'SAVING' ? 'text-amber-500 animate-pulse' : 'text-blue-900'} transition-colors`}>
                    {saveStatus === 'SYNCED' ? 'DATA_SYNCED' : saveStatus === 'SAVING' ? 'SAVING...' : 'STORAGE_OK'}
                </div>
                <div className={`w-1 h-1 rounded-full ${saveStatus === 'SYNCED' ? 'bg-green-500' : saveStatus === 'SAVING' ? 'bg-amber-500' : 'bg-blue-900'}`}></div>
            </div>
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <button 
                            onClick={() => {
                                ...
                                downloadAnchorNode.setAttribute("download", `waifu_chat_backup_${Date.now()}.json`);
                                document.body.appendChild(downloadAnchorNode);
                                downloadAnchorNode.click();
                                downloadAnchorNode.remove();
                                alert(">> SYSTEM EXPORT COMPLETE: CHAT HISTORY ARCHIVED.");
                            }} 
                            className="text-[10px] text-green-500 hover:text-white uppercase font-bold tracking-widest border border-green-900/50 px-3 py-2 bg-green-900/10 hover:bg-green-900/40 transition-colors"
                        >
                            [DOWNLOAD CHAT BACKUP (.JSON)]
                        </button>
                    </div>
                </div>
