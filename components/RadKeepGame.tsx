...
export const RadKeepGame: React.FC<Props> = ({
    ...
    const handleAction = async (actionText: string) => {
        ...
        try {
            // Note: generateAdventureTurn now uses Pro model (gemini-3.1-pro-preview)
            // for high-quality RPG responses.
            ...
            if (e.name === 'AbortError') {
                console.log(">> ADVENTURE TURN ABORTED.");
                return;
            }
                ...
                <div className="flex-1 p-4 md:p-8 overflow-y-auto custom-scrollbar flex flex-col gap-4" ref={scrollRef}>
                    ...
                    {isLoading && (
                        <div className="mt-4 flex items-center gap-4">
                            <div className="text-amber-500 animate-pulse">{`>> CALCULATING OUTCOME...`}</div>
                            <button 
                                onClick={handleCancelTurn}
                                className="text-[10px] border border-red-500 text-red-500 px-2 py-1 hover:bg-red-900/40 transition-all font-bold"
                            >
                                [ABORT TURN]
                            </button>
                        </div>
